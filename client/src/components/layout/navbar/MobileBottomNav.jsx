...
const MobileBottomNav = ({ user, profileImage, onSearchOpen, onProfileMenuOpen }) => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-t border-gray-200/60 py-3 px-6 shadow-[0_-4px_20px_rgba(0,0,0,0.04)]">
      <div className="flex items-center justify-between max-w-md mx-auto">
        ...
        <Link
          to="/wishlist"
          className="relative flex flex-col items-center justify-center gap-1 text-gray-500 hover:text-gray-900 transition-colors"
        >
          <div className="relative">
            <Heart size={20} />
            {wishlistCount > 0 && (
              <span className="absolute -top-1.5 -right-2 bg-rose-500 text-white text-[8px] font-black w-4 h-4 flex items-center justify-center rounded-full ring-2 ring-white select-none">
                {wishlistCount}
              </span>
            )}
          </div>

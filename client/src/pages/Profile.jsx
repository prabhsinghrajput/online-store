...
// Admin check moved to server - client only checks user metadata
...
const Profile = () => {
  ...
  useEffect(() => {
    const loadUserData = async () => {
      try {
        // Use getSession instead of getUser - it's more resilient
        ...
        const storedProfile = JSON.parse(localStorage.getItem(`profile_${currentUser.id}`) || '{}');
        setFormData({
          displayName: storedProfile.displayName || currentUser.user_metadata?.full_name || '',
        ...
        >
          ...
          <div className="relative flex items-center gap-5">
            <div className="relative">
              <img
                src={formData.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.displayName || 'User')}&background=6366f1&color=fff&size=128`}
                alt="Profile"
